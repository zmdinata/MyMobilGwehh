...
import CharacterAvatar, { getCharacterAvatarMeta } from './CharacterAvatar';
...
export default function DialogueModal({
  ...
  const meta = getCharacterAvatarMeta(currentLine.speaker, currentLine.mood);
  ...
  return (
    <div id="dialogueModal" className="absolute inset-0 z-40 bg-slate-950/80 backdrop-blur-sm flex items-end justify-center p-3 sm:p-6 pb-[max(1.5rem,env(safe-area-inset-bottom,1.5rem))] pointer-events-auto">
      <div className="max-w-2xl w-full bg-gradient-to-b from-slate-900 to-slate-950 border-2 border-cyan-500/80 rounded-3xl p-4 sm:p-5 shadow-2xl relative overflow-hidden flex flex-col sm:flex-row items-center sm:items-start gap-4">
        {/* Character Portrait Avatar (Hi-res Comic Art with dynamic character gradient) */}
        <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gradient-to-br ${meta.gradient} border-2 ${meta.border} flex items-center justify-center overflow-hidden flex-shrink-0 shadow-lg relative transition-all duration-300`}>
        ...
        <div className="flex-1 flex flex-col justify-between w-full min-h-[90px]">
          <div>
            <div className="flex items-center justify-between mb-1">
              <span id="dialogueSpeakerName" className={`font-fredoka text-lg ${meta.textColor || 'text-amber-300'} drop-shadow`}>
